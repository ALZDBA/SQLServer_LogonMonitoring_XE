...
DBA_FailedConnectionTracker: Maintain an overview of which connections are made to this SQLServer instance and failed
...
if 1 = 1 and object_id('dbo.T_DBA_FailedConnectionTracker') is not null
begin
	/*DROP Table dbo.T_DBA_FailedConnectionTracker;*/
	EXECUTE sp_rename N'dbo.T_DBA_FailedConnectionTracker', N'T_DBA_FailedConnectionTracker_OBSOLETE20210930', 'OBJECT' 
...
set @TraceFileName = 'C:\temp\DBA_XE_TrackFailedLogins' + '_' + replace(@@servername,'\','_') + '_' + replace(replace(replace(convert(char(10),getdate(),121),'-',''),' ','_'),':','') + '_' -- + '.trc' wordt automatisch toegevoegd
...
/****** Object:  Job [DBA_Process_FailedConnectionTracker_XE]    Script Date: 21/09/2021 7:51:15 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 21/09/2021 7:51:16 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [spc_DBA_Process_XE_ConnectionTracker]    Script Date: 21/09/2021 7:51:16 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'spc_DBA_Process_XE_ConnectionTracker', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Send email when failed]    Script Date: 21/09/2021 7:51:16 ******/
...
EXEC msdb.dbo.sp_send_dbmail 
	@recipients = ''DBA.Group@yourplace.com'', 
	@subject = @subject, 
	@body=@body;
', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Ieder kwartier', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
