...
if 1=1 and object_id( 'dbo.T_DBA_ConnectionTracker') is not null
begin
	/*drop table dbo.T_DBA_ConnectionTracker*/
	EXECUTE sp_rename N'dbo.T_DBA_ConnectionTracker', N'T_DBA_ConnectionTracker_OBSOLETE20210930', 'OBJECT' 
...
set @TraceFileName = 'C:\temp\DBA_XE_TrackLogins' + '_' + replace(@@servername,'\','_') + '_' + replace(replace(replace(convert(char(10),getdate(),121),'-',''),' ','_'),':','') + '_' -- + '.trc' wordt automatisch toegevoegd
...
CREATE or ALTER  PROCEDURE [dbo].[spc_DBA_Process_XE_ConnectionTracker]   
	@Debug BIT = 0
AS
BEGIN
/*
    ...
    IF @RwCount = 0
        BEGIN
            SELECT @XEFilename = NULL
                    , @XEfileoffset = NULL;
            --SELECT @XEFilename AS XEFilename;
        END;
	BEGIN TRY  
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'spc_DBA_Process_XE_ConnectionTracker', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Send email when failed]    Script Date: 20/09/2021 9:11:02 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Send email when failed', 
	...
	set @subject = convert(varchar(128), serverproperty(''servername'')) + '': job DBA_Process_ConnectionTracker_XE failed !''
declare @body varchar(1000)
	set @body = convert(varchar(128), serverproperty(''servername'')) + '': job DBA_Process_ConnectionTracker_XE failed and needs to be checked ! '' + convert(char(26), getutcdate(),121) + '' utc''
